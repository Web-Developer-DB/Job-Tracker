...
describe('app store', () => {
  ...
  it('rejects invalid backup imports without mutating or persisting state', () => {
    const saveMock = vi.fn<(state: AppState) => Promise<void>>(async () => undefined);
    const driver: StorageDriver = {
      load: vi.fn(async () => baseState),
      save: saveMock,
    ...
    expect(() => store.getState().importBackup({ version: '1.0', data: {} })).toThrow(/invalid backup/i);

    expect(store.getState().applications).toBe(before);
